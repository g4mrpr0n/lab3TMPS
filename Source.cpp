...
#include "shop/PaymentLibraryAdapter.h"
#include "inventory/BookProxy.h"
#include "inventory/BookInventory.h"
#include "views/InventoryDisplay.h"
#include "inventory/Ebook.h"
#include "inventory/PhysicalBook.h"
...
int main()
{
	...
	PaymentLibraryAdapter adapter(externalPayment);
	...
	BookAbstract* b1 = new BookProxy("Le big meowmeow", "Leon Jenkins", 1999, fs, 50);
	BookAbstract* b2 = new BookProxy("The bright tomorrow", "Anan Poopy", 2020, fs, 100);
	BookAbstract* b3 = new BookProxy("Hello world!", "Alex Mamia", 2009, fs, 60);
	BookAbstract* b4 = new BookProxy("Programmer jokes", "Tom Dandy", 1967, fs, 80);
	BookAbstract* b5 = new BookProxy("Le big meowmeow", "Leon Jenkins", 1999, fs, 50);

	BookAbstract* b6 = new PhysicalBook("the book!", "me", 2015, fs, 120);
	BookAbstract* b7 = new Ebook("literally meow", "stinkey", 2022, fs, 150);
	...
	while (true)
	{
		...
		std::cin >> choice;
		float rate = adapter.getRate(step1);
		...
		if (step1 == 'c')
		{
			std::cout << c->calculateTotal(c->getShoppingCartMap()[0]) * rate << " since you're a customer.";
		}
		else { std::cout << c->calculateTotal(c->getShoppingCartMap()[0]) * rate << " since you're an admin."; }
		MakePurchase m;
		...
		std::cin >> year >> price;
		BookAbstract* b = new BookProxy(name, author, year, fs, price);
		Inventory.addBook(b);
		_getch();
		break;
		}
